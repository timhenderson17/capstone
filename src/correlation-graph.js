...
function drawText(props) {
  ...
  var text = props.text;
  d3.select(selector).append('g').attr('transform', 'translate(' + xOffset + ',' + yOffset + ')').append('text').style('fill', '#ffffff').style('fill-opacity', 1).style('pointer-events', 'none').style('stroke', 'none').style('font-size', 14).style('font', 'verdena').text(text);
}
...
function render(props) {
  ...
  var parentHeight = parent.innerHeight || parent.clientHeight || 750;
  ...
  var backgroundRect = svg.append('rect').attr('width', width).attr('height', height).classed('background', true).style('fill', 'black');

  var linkWidthScale = d3.scalePow().exponent(2).domain([0, 1]).range([0, 5]);

  // http://colorbrewer2.org/?type=qualitative&scheme=Paired&n=12
  var boldAlternating12 = ['#4deeea', '#f000ff', '#ffe700', '#ff7f00', '#6a3d9a', '#b15928', '#a6cee3', '#b2df8a', '#fb9a99', '#fdbf6f', '#cab2d6', '#ffff99'];

  var gephiSoftColors = ['#4deeea', // light blue
  '#f000ff', // light green
  '#ffe700', // pink
  '#ffc482', // soft orange
  '#efc4ff', // soft violet
  ...
  var label = node.append('text').text(function (d) {
    ...
    // ),
    8) + 'px';
  }).style('fill', '#ffffff').style('fill-opacity', 1).style('pointer-events', 'none').style('stroke', 'none').attr('class', 'label').attr('dx', function (d) {
    var dxValue = -1 * (this.getComputedTextLength() / 2) + 'px';
    return dxValue;
  }).attr('dy', function (d) {
    var font_size = Math.max(Math.min(2 * nodeRadiusScale(d.linkWeightSum), (2 * nodeRadiusScale(d.linkWeightSum) - 8) / this.getComputedTextLength() * labelTextScalingFactor), 8)
    var dyValue = (nodeRadiusScale(d.linkWeightSum) + font_size) + 'px';
    return dyValue;
});
  ...
  d3.select('body').append('svg').attr('height', 100).attr('width', 960).attr('class', 'sliderTextSVG').attr('font', 'Verdena');
